...
public class FileController {
    ...
    @PostMapping("/changeHeaderPhoto")
    public R changeHeaderPhoto(@AuthenticationPrincipal LoginUserInfo userInfo,
                               MultipartFile file) {
        System.out.println("changeHeaderPhoto");
        // 检查上传的图片是否为空
        if (file.isEmpty()){
        ...
        if (!userService.updateById(new User().setId(userInfo.getId()).setHeadPhoto(imageUrl))){
            dest.delete();
            throw new UpdateExteption("修改头像失败!,请重试");
        }
